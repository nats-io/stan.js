...
function pingServerPeriodically(that) {
  that.pingTimer = timers.setTimeout(function() {
    that.pingOut++;
    if (that.pingOut > that.pingMaxOut) {
      closeDueToPing(that, new Error(MAX_PINGS_EXCEEDED));
      return;
    }
...
function cleanupOnClose(that) {
  if (that.pingTimer) {
    timers.clearTimeout(that.pingTimer);
    that.pingTimer = null;
  }

  if (that.nc) {
    if (that.ackSubscription) {
      that.nc.unsubscribe(that.ackSubscription);
      that.ackSubscription = null;
    }
    if (that.hbSubscription) {
      that.nc.unsubscribe(that.hbSubscription);
      that.hbSubscription = null;
    }
    if (that.pingSubscription) {
      that.nc.unsubscribe(that.pingSubscription);
      that.pingSubscription = null;
    }
  }
...
function closeDueToPing(that, err) {
  if (!that.nc) {
    return;
  }
