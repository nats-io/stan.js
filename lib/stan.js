...
Stan.prototype.createConnection = function() {
  ...
  this.nc.on('connect', function() {
    ...
    that.nc.requestOne(discoverSubject, new Buffer(req.serializeBinary()), 2*1000, function(msg) {
...
Stan.prototype.subscribe = function(subject, qGroup, options) {
  ...
  // fixme: hardcoded timeout
  this.nc.requestOne(this.subRequests, new Buffer(sr.serializeBinary()), 2*1000, function(msg) {
...
function closeOrUnsubscribe(that, doClose) {
  ...
  // fixme: hardcoded timeout
    sc.nc.requestOne(reqSubject, new Buffer(ur.serializeBinary()), 2*1000, function (msg) {
      if(msg instanceof nats.NatsError) {
          var type = doClose ? CLOSE_REQ_TIMEOUT : UNSUB_REQ_TIMEOUT;
          var err = new nats.NatsError(type, type, msg);
          if(msg.code === nats.REQ_TIMEOUT) {
