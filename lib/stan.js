...
Stan.prototype.stanProps = ['ackTimeout', 'connectTimeout', 'discoverPrefix', 'maxPubAcksInflight', 'nc', 'stanEncoding', 'stanMaxPingOut', 'stanPingInterval']
...
Stan.prototype.initConnection = function () {
  ...
  this.hbSubscription = this.nc.subscribe(hbInbox, (_, msg) => {
    msg.respond()
  })

  this.pingInbox = nats.createInbox()
  this.pingSubscription = this.nc.subscribe(this.pingInbox, (_, msg) => {
    if (msg) {
      const pingResponse = proto.pb.PingResponse.deserializeBinary(Buffer.from(msg.data, 'binary'))
  ...
  this.nc.request(discoverSubject, (err, msg) => {
    if (err) {
      if (err.code === nats.ErrorCode.TIMEOUT_ERR) {
    ...
    const cr = proto.pb.ConnectResponse.deserializeBinary(Buffer.from(msg.data, 'binary'))
    if (cr.getError() !== '') {
    ...
    if (cr.getProtocol() >= PROTOCOL_ONE) {
      if (cr.getPingInterval() !== 0) {
        ...
        this.pingTimer = setTimeout(function pingFun () {
          ...
          that.nc.publishRequest(that.pingRequests, that.pingInbox, that.pingBytes)
  ...
  }, Buffer.from(req.serializeBinary()), { timeout: this.options.connectTimeout })
}
...
Stan.prototype.createConnection = function () {
  ...
  if (this.nc === undefined) {
    if (this.options.encoding && this.options.encoding !== 'binary') {
      throw new Error(BINARY_ENCODING_REQUIRED)
    }
    const pl = this.options.payload
    if (pl && pl !== Payload.Binary) {
      throw new Error(BINARY_ENCODING_REQUIRED)
    }
    // copy the options
    const opts = JSON.parse(JSON.stringify(this.options))
    this.stanProps.forEach((v) => {
      delete opts[v]
    })
...
Stan.prototype.close = function () {
  ...
  if (this.nc && this.closeRequests) {
    ...
    req.setClientId(this.clientID)
    this.nc.request(this.closeRequests, (err, msg) => {
      const nc = this.nc
      delete this.nc
      let closeError = null
      ...
      if (err) {
        ...
        const cr = proto.pb.CloseResponse.deserializeBinary(Buffer.from(msg.data, 'binary'))
...
Stan.prototype.processAck = function () {
  return (_, msg) => {
    // noinspection JSUnresolvedVariable
    const pa = proto.pb.PubAck.deserializeBinary(Buffer.from(msg.data, 'binary'))
...
Stan.prototype.publish = function (subject, data, ackHandler) {
  ...
  this.nc.publishRequest(subj, this.ackSubject, bytes)
  this.pubAckOutstanding++
...
Stan.prototype.subscribe = function (subject, qGroup, options) {
  ...
  this.nc.request(this.subRequests, (err, msg) => {
    if (err) {
      if (err.code === nats.ErrorCode.REQ_TIMEOUT) {
        const werr = new nats.NatsError(SUB_REQ_TIMEOUT, SUB_REQ_TIMEOUT, err)
        retVal.emit('timeout', werr)
      } else {
    ...
    // noinspection JSUnresolvedVariable
    const r = proto.pb.SubscriptionResponse.deserializeBinary(Buffer.from(msg.data, 'binary'))
    err = r.getError()
    if (err && err.length !== 0) {
      retVal.emit('error', new Error(err))
      retVal.inboxSub.unsubscribe()
      retVal.emit('unsubscribed')
      return
    }
    retVal.ackInbox = r.getAckInbox()
    retVal.emit('ready')
  }, Buffer.from(sr.serializeBinary()), { timeout: this.options.connectTimeout })
...
Subscription.prototype.closeOrUnsubscribe = function (doClose) {
  ...
  sc.nc.request(reqSubject, (err, msg) => {
    if (err) {
      const type = doClose ? CLOSE_REQ_TIMEOUT : UNSUB_REQ_TIMEOUT
      const werr = new nats.NatsError(type, type, err)
      if (err.code === nats.ErrorCode.REQ_TIMEOUT) {
        this.emit('timeout', werr)
      } else {
  ...
  }, Buffer.from(ur.serializeBinary()), { timeout: sc.options.connectTimeout })
}
...
Stan.prototype.processMsg = function () {
  // curry
  return (k, msg) => {
    const sub = this.subMap[msg.subject]
    try {
      // noinspection JSUnresolvedVariable
      const payload = proto.pb.MsgProto.deserializeBinary(Buffer.from(msg.data, 'binary'))
      if (sub === undefined || !this.nc) {
        return
      }
      const m = new Message(this, payload, sub)
      sub.emit('message', m)
      m.maybeAutoAck()
    } catch (error) {
...
Message.prototype.ack = function () {
  // it is possible for the client to call `close` during
  // the event loop of the message handler
