...
BAD_SUBJECT = 'STAN_BAD_SUBJECT',
BAD_SUBJECT_MSG = 'stan: subject must be supplied',
BAD_CLUSTER_ID = 'STAN_BAD_CLUSTER_ID',
BAD_CLUSTER_ID_MSG = 'stan: cluster ID must be supplied',
BAD_CLIENT_ID = 'STAN_BAD_CLIENT_ID',
BAD_CLIENT_ID_MSG = 'stan: client ID must be supplied',
ACK_TIMEOUT = 'STAN_ACK_TIMEOUT',
ACK_TIMEOUT_MSG = 'stan: publish ack timeout',
MAX_FLIGHT_LIMIT_REACHED = 'STAN_MAX_FLIGHT_LIMIT_REACHED',
MAX_FLIGHT_LIMIT_REACHED_MSG = 'stan: max in flight reached.',
CONN_CLOSED = 'STAN_CONN_CLOSED',
CONN_CLOSED_MSG = 'stan: Connection closed',
BINARY_ENCODING_REQUIRED = 'STAN_BIN_ENCODING_REQUIRED',
BINARY_ENCODING_REQUIRED_MSG = 'stan: NATS connection encoding must be \'binary\'.';
...
exports.BAD_CLUSTER_ID = BAD_CLUSTER_ID
exports.BAD_CLIENT_ID = BAD_CLIENT_ID
exports.ACK_TIMEOUT = ACK_TIMEOUT
exports.MAX_FLIGHT_LIMIT_REACHED = MAX_FLIGHT_LIMIT_REACHED
exports.CONN_CLOSED = CONN_CLOSED
exports.BINARY_ENCODING_REQUIRED = BINARY_ENCODING_REQUIRED


function Stan(clusterID, clientID, opts) {
  events.EventEmitter.call(this);
  if(typeof clusterID !== 'string' || clusterID.length < 1) {
    throw new Error(BAD_CLUSTER_ID_MSG, BAD_CLUSTER_ID);
  }
  if(typeof clientID !== 'string' || clientID.length < 1) {
    throw new Error(BAD_CLIENT_ID_MSG, BAD_CLIENT_ID);
  }
...
Stan.prototype.createConnection = function() {
  ...
  if(typeof this.options.nc === 'object') {
    if(this.options.nc.encoding !== 'binary') {
     throw new Error(BINARY_ENCODING_REQUIRED_MSG, BINARY_ENCODING_REQUIRED);
    } else {
...
Stan.prototype.publish = function(subject, data, ackHandler) {
  if (this.nc === undefined) {
    if (util.isFunction(ackHandler)) {
      ackHandler(new Error(CONN_CLOSED_MSG, CONN_CLOSED));
    } else {
      throw new Error(CONN_CLOSED_MSG_MSG, CONN_CLOSED);
    }
  }
  ...
  if(this.pubAckOutstanding > this.options.maxPubAcksInflight) {
    ...
    if(util.isFunction(ackHandler)) {
      ackHandler(new Error(MAX_FLIGHT_LIMIT_REACHED_MSG, MAX_FLIGHT_LIMIT_REACHED));
    } else {
      throw new Error(MAX_FLIGHT_LIMIT_REACHED_MSG, MAX_FLIGHT_LIMIT_REACHED);
    }
  }
  ...
  ack.t = timers.setTimeout(function() {
    ...
    if(_ack && _ack.ah !== undefined) {
      _ack.ah(new Error(ACK_TIMEOUT_MSG, ACK_TIMEOUT), peGUID);
    }
...
Stan.prototype.subscribe = function(subject, qGroup, options) {
  ...
  if (typeof subject !== 'string' || subject.length === 0) {
    process.nextTick(function() {
      that.emit('error', new Error(BAD_SUBJECT_MSG, BAD_SUBJECT));
    });
    return retVal;
  }
  ...
  if(this.isClosed()) {
    process.nextTick(function() {
      that.emit('error', new Error(CONN_CLOSED_MSG, CONN_CLOSED));
    });
    return retVal;
  }
...
Subscription.prototype.unsubscribe = function() {
  ...
  if(sc.isClosed()) {
    this.emit('error', new Error(CONN_CLOSED_MSG, CONN_CLOSED));
    return;
  }
  ...
  sc.nc.timeout(sid, 2 * 1000, 1, function () {
    that.emit('timeout', new Error(ACK_TIMEOUT_MSG, ACK_TIMEOUT));
  });
};
