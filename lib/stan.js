...
Stan.prototype.initConnection = function () {
  ...
  this.pingSubscription = this.nc.subscribe(this.pingInbox, (msg) => {
    if (msg) {
      ...
      if (err) {
        this.closeWithError('connection_lost', new Error(err))
        return
      }
    }
    this.pingOut = 0
  ...
  this.nc.requestOne(discoverSubject, Buffer.from(req.serializeBinary()), this.options.connectTimeout, (msg) => {
    ...
    if (cr.getError() !== '') {
      this.closeWithError('error', new Error(cr.getError()))
      return
    }
...
Stan.prototype.setupHandlers = function () {
  ...
  this.nc.on('permission_error', (err) => {
    this.emit('permission_error', err)
  })
}
...
Stan.prototype.processAck = function () {
  return (msg) => {
    ...
    if (a && a.ah) {
      ...
      a.ah(err === '' ? undefined : new Error(err), guid)
    }
  }
}
...
exports.NewSubscriptionOptions = function () {
