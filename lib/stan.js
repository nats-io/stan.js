...
Stan.prototype.subscribe = function(subject, qGroup, options) {
  ...
  var rsid = this.nc.request(this.subRequests, new Buffer(sr.serializeBinary()), {'max': 1}, function(msg) {
    ...
    retVal.emit('ready');
    if(retVal.pending) {
      // FIXME: this is to handle the unknown case where we are getting messages before protocol response
      for (var i=0; i < retVal.length; i++) {
        retVal[i].emit('message', msg);
        retVal[i].maybeAutoAck();
      }
      delete retVal.pending;
    }
  });
