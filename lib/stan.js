        ...
        if (cr.getPingInterval() !== 0) {
          ...
          that.pingTimer = setTimeout(function pingFun() {
            ...
            if (that.pingOut > that.pingMaxOut) {
              that.closeDueToPing(new Error(MAX_PINGS_EXCEEDED));
              return;
            }
...
/**
 * Close stan with an error - this should only be with errors
 * not related to the stan pings. Closes calling error handler
 * and close handler.
 * @see Stan#closeDueToPing()
 * @param event
 * @param error
 * @private
 */
...
/**
 * Close the stan connection if there's a ping error. Note
 * that while the protocol subscriptions/pings/timers are closed
 * the stan protocol close is not sent as this could
 * potentially disconnect a different client that connected
 * with the same ID while partitioned from the server.
 *
 * @param error
 * @private
 */
Stan.prototype.closeDueToPing = function(error) {
